...
# Try to get Messages notification count from dock badge
UNREAD_COUNT=$(osascript << 'EOF'
try
    tell application "System Events"
        tell application process "Dock"
            try
                set messagesTile to (first UI element of list 1 whose name contains "Messages")
                try
                    set badgeText to value of attribute "AXStatusLabel" of messagesTile
